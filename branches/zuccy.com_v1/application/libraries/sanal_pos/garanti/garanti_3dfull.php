 ...
 **/
 ...
 /*
	...
	Php Kodu
	$olustur->{'3dfull'}->merchantid	= '';
	$olustur->{'3dfull'}->terminalid	= '';
	$olustur->{'3dfull'}->storekey		= '';
	$olustur->{'3dfull'}->username		= '';
	$olustur->{'3dfull'}->password		= '';
	echo serialize($olustur);
 */

class garanti_3dfull
{
	public $ci;
	protected $banka = 'garanti';
	protected $ascii = '3dfull';
	...
	{
		log_message('debug', 'garanti_3dfull Library Yüklendi');
	...
	function form_gonder($gelen_veriler = null)
	{
		if(!is_null($gelen_veriler) AND $banka_bilgi = $this->banka_bilgi())
		{
			...
			$form .= form_hidden('secure3dsecuritylevel', '3D_FULL');
