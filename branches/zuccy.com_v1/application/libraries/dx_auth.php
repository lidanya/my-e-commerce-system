...
class dx_auth
{
	...
	*/
	...
	{
		$this->ci->load->model('dx_auth/users', 'users');
		$sorgu = $this->ci->users->getProfile($this->getUserID());
		$goster = $sorgu->row();
		
		return $goster->$istenen;
	}

	function getUserPVeri_id($istenen, $id)
	{
		$this->ci->load->model('dx_auth/users', 'users');
		$sorgu = $this->ci->users->getProfile($id);
		$goster = $sorgu->row();
	...
	{
		return $this->ci->session->userdata('DX_parent_roles_id');
	}
	...
	function get_parent_role_name()
	{
		return $this->ci->session->userdata('DX_parent_roles_name');
	}
	...
	function is_role($roles = array(), $use_role_name = TRUE, $check_parent = TRUE)
	{
		...
		if ($use_role_name)
		{
			if($this->ci->session->userdata('DX_role_name')) {
				array_push($check_array, $this->ci->session->userdata('DX_role_name'));
			}
		}
		else
		{
			if($this->ci->session->userdata('DX_role_id')) {
	...
	/* eski yazdığım bir aralar serdar bey durdurun dedi öldürecektim az daha :D
	function is_role($roles = array(), $use_role_name = TRUE, $check_parent = TRUE, $user_id = null)
	{
		// Default return value
		$result = FALSE;
		...
		if(!is_null($user_id))
		{
			...
			$user_sorgu = $this->ci->db->get_where('users', array('id' => $user_id), 1);
			if($user_sorgu->num_rows() > 0)
			{
				$user_bilgi = $user_sorgu->row();
				$user_roles_name = $user_bilgi->name;
			} else {
				$result = FALSE;
				$user_roles_name = NULL;
			}
		} else {
			$user_roles_name = $this->ci->session->userdata('DX_parent_roles_name');
		}
	...
	*/
	...
	public function login_by_id($user_id, $remember = TRUE)
	{
		// Load Models
		$this->ci->load->model('dx_auth/users', 'users');
		$this->ci->load->model('dx_auth/user_temp', 'user_temp');
		...
		$get_user_function = 'get_user_by_id';
	
		// Get user query
		if ($query = $this->ci->users->$get_user_function($user_id) AND $query->num_rows() == 1)
		{
			// Get user record
			$row = $query->row();
	...
	function register($username, $password, $email, $activation=false, $roleID = false)
	{		
		// Load Models
		$this->ci->load->model('dx_auth/users', 'users');
		$this->ci->load->model('dx_auth/user_temp', 'user_temp');
		...
		if(!$roleID)
		{
			$roleID = (config('site_ayar_varsayilan_mus_grub')) ? config('site_ayar_varsayilan_mus_grub'):4;
		}
		...
		{				
			...
			$son_id = $this->ci->db->insert_id();
			$new_user['sonid'] = $son_id;
			// Trigger event
			$this->ci->dx_auth_event->user_activated($son_id);	
			$this->ci->dx_auth_event->user_activated_2($son_id);
		...
		if ($insert)
		{
			...
			{
				// Create email
				$from = $this->ci->config->item('site_ayar_email_cevapsiz');
				$subject = $this->ci->lang->line('auth_activate_subject');
			...
			else
			{
				...
				if (($this->ci->config->item('DX_email_account_details')))
				{
					// Create email
					$from = $this->ci->config->item('site_ayar_email_cevapsiz');
					$subject = $this->ci->lang->line('auth_account_subject');
					
					$mail_data['email'] 	= $new_user['email'];
					$mail_data['password'] 	= $new_user['password'];
					$mail_data['adsoyad'] 	= $new_user['email'];
					$mail_data['mesaj'] 	= 'Tebrikler! Kayıt işlemleriniz başarı ile gerçekleştirildi. Aşağıdaki bilgileri kullanarak sistemimize <a href="'.base_url().'uye/giris" style="color:#008fff !important">buradan</a> Giriş yapabilirsiniz.';
					
					if($this->ci->input->post("adiniz") != '' && $this->ci->input->post("soyadiniz") != '')
					{
						$mail_data['adsoyad'] 	= $this->ci->input->post("adiniz"). ' ' . $this->ci->input->post("soyadiniz");
						$mail_data['mesaj'] 	= 'Tebrikler! Kayıt işlemleriniz başarı ile gerçekleştirildi. Aşağıdaki bilgileri kullanarak sistemimize <a href="'.base_url().'uye/giris" style="color:#008fff !important">buradan</a> Giriş yapabilirsiniz: ';
					}
					
					$message = $this->ci->load->view(tema() . 'mail_sablon/uyelik/uye_kayit', $mail_data, true);
		...
		if ($login)
		{
			...
			if ($query = $this->ci->users->get_login($login) AND $query->num_rows() == 1)
			{
				...
				if ( ! $row->newpass_key)
				{
					...
					//fuck yourself
					$kullanici_inf_sorgu = $this->ci->db->get_where('usr_ide_inf', array('user_id' => $row->id), 1);
					
					// Create email
					$from = config('site_ayar_email_cevapsiz');
					$subject = 'Şifreniz Değiştirilmiştir';
					$mail_data['link']				= $data['reset_password_uri'];
					$mail_data['sifre']				= $data['password'];
					$mail_data['adsoyad']			= $this->ci->input->post('eposta');
					if( $kullanici_inf_sorgu->row()->ide_adi != '' && $kullanici_inf_sorgu->row()->ide_soy != '')
					{
						$mail_data['adsoyad']		= $kullanici_inf_sorgu->row()->ide_adi . ' ' . $kullanici_inf_sorgu->row()->ide_soy;
					}

					$message = $this->ci->load->view(tema() . 'mail_sablon/uyelik/uye_sifre_sifirla', $mail_data, true);
	...
	{		
		...
		if ($query = $this->ci->user_temp->activate_user($username, $key) AND $query->num_rows() > 0)
		{
			...
			if ($this->ci->users->create_user($row))
			{
				...
				$this->ci->dx_auth_event->user_activated_2($this->ci->db->insert_id());
				$this->ci->dx_auth_event->user_activated_3($this->ci->db->insert_id());
	...
	function is_ajax_giris_captcha_match($code)
	{
		$result = strtolower($code) == strtolower($this->ci->ng_session->userdata('uye_giris'));
		
		return $result;
	}

	function is_ajax_kayit_captcha_match($code)
	{
		$result = strtolower($code) == strtolower($this->ci->ng_session->userdata('uye_kayit'));
		return $result;
	}
}