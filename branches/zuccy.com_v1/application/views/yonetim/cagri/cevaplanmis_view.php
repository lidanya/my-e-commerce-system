...
$link = yonetim_url('cagri/cevaplanmis/listele/'.$sort.'_'.$order_lnk.'/'.$filt_lnk.'/'.$page_lnk);
...
<div class="box">
	...
	<div class="left"></div>
  	...
  	<div class="heading">
    	<h1 style="background-image: url('<?php echo yonetim_resim(); ?>mail.png');">Cevaplanmış Çağrılar</h1>
  	</div>
	<div class="content">

<form action="<?php echo yonetim_url('cagri/cevaplanmis/listele'); ?>" method="post" enctype="multipart/form-data" id="form">
	<table class="list">
        <thead>
          <tr>
         ...
         ?>
          <tr>
            ...
            <td class="left"><a href="<?php echo yonetim_url('cagri/cevap_yaz/index/'. $r->ticket_id); ?>?geri_don=<?php echo yonetim_url('cagri/cevaplanmis/listele'); ?>"><?php echo $r->ticket_konu;?></a></td>
			<td class="right"><?php echo ($r->ticket_adm_durum == 2) ? 'Okunmuş' : 'Okunmamış';?></td>
            <td class="center"><?php echo date('Y.m.d',$r->ticket_tarih);?></td>
            <td class="right"> [ <a href="<?php echo yonetim_url('cagri/cevap_yaz/index/'. $r->ticket_id); ?>?geri_don=<?php echo yonetim_url('cagri/cevaplanmis/listele'); ?>">Cevap Yaz</a> ]</td>
          </tr>
          <?php
          endforeach;
          ?>
...
function filter(islem) {
	if(islem == 1)
	{
		url = "<?php echo yonetim_url('cagri/cevaplanmis/listele/'. $sort_lnk); ?>/";
	}
...
function coklu_islem(islem,val)
{
	...
	if(toplam > 0)
	{
		...
		{
			$('#form').attr('action','<?php echo yonetim_url("cagri/cevaplanmis/arsive_ekle"); ?>').submit();
		}
	}else{
