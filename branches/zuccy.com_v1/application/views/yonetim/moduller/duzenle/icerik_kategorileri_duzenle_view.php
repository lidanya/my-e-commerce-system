<?php 
	...
	$rules['ozellik_kategori_id']					= 'trim|required|numeric|xss_clean';
	...
	$fields['ozellik_kategori_id']					= 'Modül Kategori';
	$fields['ozellik_siralama_limit']				= 'Modül Sıralama Limiti';
	...
	if($val->run())
	{
		...
		$this->db->where('eklenti_ascii', $modul->eklenti_ascii);
		$this->db->where('ayar_adi', 'kategori_id');
		if($this->db->update('eklentiler_ayalar', array('ayar_deger' => $val->ozellik_kategori_id)))
		{
			$eklenti_durum = true;
		}
				...
				<tr>
					...
					<td>
						<?php
							...
							$allowed_categories			= array('0' => ' - Seçiniz - ');
							if($categories) {
							...
							/* categories */
							$secili = eklenti_ayar($modul->eklenti_ascii, 'kategori_id');
							echo form_dropdown('ozellik_kategori_id', $allowed_categories, $secili);
						?>
					</td>
				</tr>
				<tr>
