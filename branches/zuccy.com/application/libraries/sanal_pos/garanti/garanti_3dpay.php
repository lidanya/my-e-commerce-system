 ...
 **/
 ...
 /*
	SQL Kodu
	O:8:"stdClass":1:{s:5:"3dpay";O:8:"stdClass":5:{s:10:"merchantid";s:0:"";s:10:"terminalid";s:0:"";s:8:"storekey";s:0:"";s:8:"username";s:0:"";s:8:"password";s:0:"";}}
	Php Kodu
	$olustur->{'3dpay'}->merchantid	= '';
	$olustur->{'3dpay'}->terminalid	= '';
	$olustur->{'3dpay'}->storekey	= '';
	$olustur->{'3dpay'}->username	= '';
	$olustur->{'3dpay'}->password	= '';
...
class garanti_3dpay
{
	...
	{
		log_message('debug', 'garanti_3dpay Library Yüklendi');
	...
	function form_gonder($gelen_veriler = null)
	{
		if(!is_null($gelen_veriler) AND $banka_bilgi = $this->banka_bilgi())
		{
			...
			$form .= form_hidden('secure3dsecuritylevel', '3D_PAY');
	...
	function form_sonuc($gelen_veriler = null)
	{
		if(!is_null($gelen_veriler) AND $banka_bilgi = $this->banka_bilgi())
		{
			...
			$gelen_veriler->kart_numarasi_yil = isset($gelen_veriler->post_verileri['kredi_kart_yil']) ? substr($gelen_veriler->post_verileri['kredi_kart_yil'], -2) : 0;
			...
			if ($strmdstatus == "1" || $strmdstatus == "2" || $strmdstatus == "3" || $strmdstatus == "4") 
			{
				if($gelen_veriler->post_verileri['response'] == 'Approved')
				{
					...
					$gonder->kod   = '';
					$gonder->veri  = '';
					$gonder->mesaj = (isset($gelen_veriler->post_verileri['errmsg']) AND $gelen_veriler->post_verileri['errmsg'] != '') ? $gelen_veriler->post_verileri['errmsg'] : $strmdstatus_mesaj;
				}
			} else {
				$gonder->durum	= false;
				$gonder->mesaj	= (isset($gelen_veriler->post_verileri['errmsg']) AND $gelen_veriler->post_verileri['errmsg'] != '') ? $gelen_veriler->post_verileri['errmsg'] : $strmdstatus_mesaj;
