...
class eklentiler_populer_urunler_model extends CI_Model
{
	function __construct()
	{
		parent::__construct();
		log_message('debug', 'Popüler Ürünlerimiz Model Yüklendi');
	}

	function yeni_urunler_listele()
	{
		if(eklenti_ayar('populer_urunler', 'siralama_sekli') != NULL)
		{
			$order = eklenti_ayar('populer_urunler', 'siralama_sekli');
		} else {
			$order = 'desc';
		}

		if(eklenti_ayar('populer_urunler', 'siralama_limit') != NULL)
		{
			$limit = eklenti_ayar('populer_urunler', 'siralama_limit');
		} else {
			$limit = 5;
		}
