...
$link = yonetim_url('cagri/arsivdeki/listele/'.$sort.'_'.$order_lnk.'/'.$filt_lnk.'/'.$page_lnk);
?>
...
<div class="box">
	...
	<div class="left"></div>
  	<div class="right"></div>
  	<div class="heading">
    	<h1 style="background-image: url('<?php echo yonetim_resim(); ?>mail.png');">Arşivdeki Çağrılar</h1>
  	</div>
	<div class="content">
        ...
        <thead>
         ...
         ?>
          <tr>
            ...
            <td class="left"><a href="<?php echo yonetim_url('cagri/cevap_yaz/index/'. $r->ticket_id); ?>?geri_don=<?php echo yonetim_url('cagri/arsivdeki/listele'); ?>"><?php echo $r->ticket_konu;?></a></td>
			<td class="right"><?php echo ($r->ticket_adm_durum == 2) ? 'Okunmuş' : 'Okunmamış';?></td>
            <td class="center"><?php echo date('Y.m.d',$r->ticket_tarih);?></td>
            <td class="right"> [ <a href="<?php echo yonetim_url('cagri/arsivdeki/arsivden_cikart/'. $r->ticket_id); ?>?geri_don=<?php echo yonetim_url('cagri/arsivdeki/listele'); ?>">Arşivden Çıkart</a> ]</td>
          </tr>
          <?php 
          endforeach;
          ?>
         <tr class="filter">
         	...
         	<td colspan="6">  <a href="javascript:void(0);" onclick="$('input[name*=\'select2\']').attr('checked', true);">Tümünü Seç</a> &nbsp;/&nbsp; <a href="javascript:void(0);" onclick="$('input[name*=\'select2\']').attr('checked', false);">Tümünü Kaldır</a>
         		<select name="eylem2" onchange="coklu_islem('select2',this.value);">
         			...
         			<option value="1">Arşivden Çıkart</option>
         		</select>
         	</td>
         </tr>
         </tbody>
...
function filter(islem) {
	if(islem == 1)
	{
		url = "<?php echo yonetim_url('cagri/arsivdeki/listele/'. $sort_lnk); ?>/";
	}
...
function coklu_islem(islem,val)
{
	...
	if(toplam > 0)
	{
		...
		{
			$('#form').attr('action','<?php echo yonetim_url("cagri/arsivdeki/arsivden_cikart"); ?>').submit();
		}
	}else{
