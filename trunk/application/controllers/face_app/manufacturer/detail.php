...
class detail extends Face_Controller {
	...
	function index($seo, $sort_link = 'price-asc', $page = 0)
	{
		...
		if($check) {
			...
			$this->template->set_master_template(face_tema() . 'manufacturer/detail');
			...
			$this->template->write_view('content', face_tema() . 'manufacturer/detail_content', $content_data);
