		...
		<form action="<?php echo yonetim_url('moduller/modul/duzenle/'. $modul->eklenti_id); ?>" method="post" enctype="multipart/form-data" id="form">
			<table class="form">
				...
				<tr>
					...
					<td>
					<?php
						$_posizyon_yerler = array('Anasayfa','Ust');
						...
						{
							$uyari ='';
							if($_posizyon_yer =='Anasayfa')
							$uyari = ' (Resim boyutu max 1000px genişliğinde olmalıdır.) ';
							if($_posizyon_yer =='Ust')
							$uyari = ' (Resim boyutu max 750px genişliğinde olmalıdır.) ';
							$checked = (in_array(strtolower($_posizyon_yer), $posizyon_yerler)) ? 'checked="chedked"':NULL;
							echo '<div style="margin: 5px;"><input type="checkbox" name="modul_posizyon[]" value="'. strtolower($_posizyon_yer) .'" '. $checked .' /> '. $_posizyon_yer .$uyari.'</div>' . "\n";
						}
					?>
					</td>
				</tr>
				...
				<tr>
					<td>Sisteme Slider Eklemek İçin :</td> 
					<td>
						<a href="<?php echo yonetim_url('moduller/slider'); ?>" style="font-weight:bold;">Buradan Slider'larınızı Yönetebilirsiniz</a>
					</td>
				</tr>
			</table>
		</form>
