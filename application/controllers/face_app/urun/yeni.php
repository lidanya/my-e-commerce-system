...
class yeni extends Face_Controller
{
	...
	function index($sort_link = 'price-asc', $page = 0)
	{
		...
		if($new_products) {
			$content_data['pagination'] = create_pagination(face_site_url('urun/yeni/index/' . $sort_link), $new_products['total'], $limit, $uri_segment, 'face');
		} else {
			$content_data['pagination'] = FALSE;
		}

		$this->template->set_master_template(face_tema() . 'urun/yeni');
		$this->template->add_region('content');
		$this->template->write_view('content', face_tema() . 'urun/yeni_content',$content_data);
